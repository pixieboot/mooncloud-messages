...
const empty_tabs_controller_html_js_1 = require("./empty_tabs_controller.html.js");
            ...
            if (loaded === "0") {
                for (let i = 0; i < chatData.messages.length; i++) {
                    if (chatData.messages[i].username === socket.username) {
                        ...
                        appendReceivedMessage(chatData.messages[i].message, chatData.messages[i].username, chatData.chatID, chatData.messages[i].message_id, chatData.messages[i].dateSent, chatData.messages[i].dateRead);
                    }
                }
...
function chatsListLoad(chatsData, user, friend, request) {
    ...
    chatTabContent.innerHTML = userDetails
        ...
        .join(" ");
    const triggerTabList = document.querySelectorAll("#chat_list_group div#friend_list_row");
    ...
    messagesTabActive();
    (0, empty_tabs_controller_html_js_1.chatTabRefresh)();
}
...
function notificationsController(data) {
    ...
    (0, empty_tabs_controller_html_js_1.notificationsTabRefresh)();
}
...
function removeNotification(result) {
    ...
    for (let i = 0; i < friendListRow.length; i++) {
        let username = friendListRow[i].childNodes[1].childNodes[1].childNodes[3].childNodes[1]
...
function checkIfNotificationsAreEmpty() {
    ...
    notificationsTab.classList.add("d-flex", "flex-column", "justify-content-center", "px-5", notificationsTab.children.length <= 1);
...
function updateFriendList(friends) {
    ...
    (0, empty_tabs_controller_html_js_1.friendListTabRefresh)();
}
...
function avatarPreviewController() {
    let settingsAvatarPreview = document.getElementById("settings_avatar_form_control");
    let settingsUploadedUserAvatar = document.getElementById("settings_uploaded_user_avatar");
    let settingsRemoveUploadedUserAvatar = document.getElementById("settings_remove_uploaded_user_avatar");
    let settingsErrorMessage = document.getElementById("settings_avatar_error_message");
...
function userSettingsController() {
    ...
    const textareaMaxCharWarning = document.getElementById("textarea_max_char_warning");
    ...
    const usernameInput = document.getElementById("settings_username_form_control");
    const usernameNotification = document.getElementById("settings_username_notification");
    ...
    updateProfileBtn.addEventListener("click", () => {
        const avatarUpload = document.getElementById("settings_uploaded_user_avatar");
        const currentAvatar = document.getElementById("settings_avatar_form_control");
        ...
        if (usernameValue ||
            ...
            avatarValue ||
            imageBase64Data) {
            socket.emit("update-user-profile", socket.username, usernameValue, emailValue, bioValue, avatarValue, imageBase64Data);
        }
    });
}
...
function changePasswordController() {
    let passwordFields = document.querySelectorAll("#password_change_fields input");
    ...
    let newPasswordToggler = document.getElementById("settings_change_password_toggler");
    let oldPasswordToggler = document.getElementById("settings_change_old_password_toggler");
    ...
    let newPasswordSpanInvalidFeedback = document.getElementById("new_password_invalid_feedback");
    let confirmNewPasswordSpanInvalidFeedback = document.getElementById("confirm_new_password_invalid_feedback");
    for (let i = 0; i < fieldsArray.length; i++) {
        fieldsArray[i].addEventListener("keyup", () => {
            ...
            let isPasswordChangeValidated = passwordValidation(newPasswordField, repeatNewPasswordField, newPasswordSpanInvalidFeedback, confirmNewPasswordSpanInvalidFeedback);
    ...
    updatePasswordBtn.addEventListener("click", () => {
        ...
        updatePasswordBtn.disabled = true;
        socket.emit("old-password-change-input", socket.username, oldPasswordValue, newPasswordValue);
        inputCoolDown();
    });
    ...
    let closeModalButtons = document.querySelectorAll("[close-password-change-modal]");
...
function passwordValidation(password, confirm_password, passwordElem, confirmPasswordElem) {
    let errors = [];
...
function oldPasswordFeedback(feedback) {
    let oldPasswordInput = document.getElementById("old_password");
    let oldPasswordSpanInvalidFeedback = document.getElementById("old_password_invalid_feedback");
    let successFeedback = document.getElementById("settings_password_change_success");
    if (feedback === false) {
